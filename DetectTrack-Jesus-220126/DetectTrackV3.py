'''
 # @ Author: J.N. Hayek
 # @ Create Time: 2022-01-28 22:40:44
 ...
 As tracking means, it uses the OpenCV library's tracking algorithms, the choosing resides in lib. "Tracking"
'''
import cv2 
from datetime import datetime
from Mods.Tracking import *
...
TrackerTypeSelection = TrackerTypes[1]
...
while True:
    ...
    # press 'c' to increase the CTimes counter and start a timer
        ...
        MT_Particles = cv2.legacy_MultiTracker()
    ...
    if CTimes!=0:
        ...
        if (diff <= duration):
            ...
            for x, y, w, h in mps:
                ...
                bbox= [x, y, x+w_0, y+h_0]

                MT_Particles.add(createTrackerByName(TrackerTypeSelection), img, bbox)
            
            success, boxes = MT_Particles.update(img)
            for i, newbox in enumerate(boxes):
                
                centroid = (int(newbox[0] + 7), int(newbox[1] + 7))
                print(centroid[0])
                text = "{}".format(i)
                
                cv2.circle(img, (centroid[0], centroid[1]), 4, (0, 255, 0), -1)
                cv2.putText(img, text, (centroid[0], centroid[1]), cv2.FONT_HERSHEY_COMPLEX, 0.3, (0, 0, 0), 1)
